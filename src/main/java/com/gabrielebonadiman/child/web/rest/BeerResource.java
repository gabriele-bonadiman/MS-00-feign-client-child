...
import com.gabrielebonadiman.child.web.rest.errors.BadRequestAlertException;
import com.gabrielebonadiman.child.web.rest.util.HeaderUtil;
...
public class BeerResource {
    ...
    /**
     * GET  /beers/askParent : get the beer asking the parent.
     *
     * @param name the id of the beer to ask parent for information
     * @return the ResponseEntity with status 200 (OK) and with body the beer, or with status 404 (Not Found)
     */
    @GetMapping("/beers/parent/{name}")
    @Timed
    public ResponseEntity<BeerDTO> getBeerFromParent(@PathVariable String name) {
        log.debug("REST request to get Beer : {}", name);
        BeerDTO beerDTO = beerService.getBeerFromParent(name);
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(beerDTO));
    }
}
