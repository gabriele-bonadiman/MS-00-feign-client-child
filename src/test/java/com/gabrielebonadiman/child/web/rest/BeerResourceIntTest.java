package com.gabrielebonadiman.child.web.rest;
...
import com.gabrielebonadiman.child.web.rest.errors.ExceptionTranslator;
...
import static com.gabrielebonadiman.child.web.rest.TestUtil.createFormattingConversionService;
...
public class BeerResourceIntTest {
    ...
    private static final String NON_EXISTING_BEER_NAME = "JustCider";
    ...
    private BeerDTO generateBeerDTO(){
        ...
        beerDTO.setAbv(DEFAULT_ABV);
        beerDTO.setIbu(DEFAULT_IBU);
        beerDTO.setIdentification(DEFAULT_IDENTIFICATION);
        beerDTO.setName(DEFAULT_BEER_NAME);
        beerDTO.setStyle(DEFAULT_STYLE);
        return beerDTO;
    }

    private BeerDTO generateEmptyBeerDTO(){
        BeerDTO beerDTO = new BeerDTO();
        beerDTO.setAbv(0.0);
        beerDTO.setIbu(0);
        beerDTO.setIdentification(0);
        beerDTO.setName("");
        beerDTO.setStyle("");
        return beerDTO;
    }
    ...
    @Test
    @Transactional
    public void getBeerFromParentWrongMethod() throws Exception {
        // Get the beer
        restBeerMockMvc.perform(post("/api/beers/parent/{name}", DEFAULT_BEER_NAME))
            .andExpect(status().isMethodNotAllowed());
    }

    @Test
    @Transactional
    public void getBeerFromParentBeerDoesExists() throws Exception {
        given(parentMicroserviceClient.getBeerByName(DEFAULT_BEER_NAME))
            .willReturn(generateBeerDTO());

        // Get the beer
        restBeerMockMvc.perform(get("/api/beers/parent/{name}", DEFAULT_BEER_NAME))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.abv").value(DEFAULT_ABV))
            .andExpect(jsonPath("$.ibu").value(DEFAULT_IBU))
            .andExpect(jsonPath("$.identification").value(DEFAULT_IDENTIFICATION))
            .andExpect(jsonPath("$.name").value(DEFAULT_BEER_NAME))
            .andExpect(jsonPath("$.style").value(DEFAULT_STYLE));
    }

    @Test
    @Transactional
    public void getBeerFromParentBeerDoesNotExists() throws Exception {
        given(parentMicroserviceClient.getBeerByName(NON_EXISTING_BEER_NAME))
            .willReturn(generateEmptyBeerDTO());

        // Get the beer
        restBeerMockMvc.perform(get("/api/beers/parent/{name}", NON_EXISTING_BEER_NAME))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.abv").value(0.0))
            .andExpect(jsonPath("$.ibu").value(0))
            .andExpect(jsonPath("$.identification").value(0))
            .andExpect(jsonPath("$.name").isEmpty())
            .andExpect(jsonPath("$.style").isEmpty());
    }

    @Test
    @Transactional
    public void getBeerFromParentReturnNullObject() throws Exception {
        // Get the beer
        restBeerMockMvc.perform(get("/api/beers/parent/{name}", DEFAULT_BEER_NAME))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.abv").doesNotExist())
            .andExpect(jsonPath("$.ibu").doesNotExist())
